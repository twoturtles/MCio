...
record ObservationPacket(
  ...
  ObservationPacket {
    ...
    Validate.check(
        cursor_mode == GLFW.GLFW_CURSOR_DISABLED || cursor_mode == GLFW.GLFW_CURSOR_NORMAL,
        "Invalid cursorMode");
...
@JsonTypeInfo(use = Id.MINIMAL_CLASS, include = As.PROPERTY, property = MCioConfig.MCIO_TYPE)
record InventorySlot(int slot, String id, int count) {}
...
@JsonTypeInfo(use = Id.MINIMAL_CLASS, include = As.PROPERTY, property = MCioConfig.MCIO_TYPE)
record StatCategory(String category, ArrayList<Stat> stats) {}

@JsonTypeInfo(use = Id.MINIMAL_CLASS, include = As.PROPERTY, property = MCioConfig.MCIO_TYPE)
record Stat(String id, int value) {}

@JsonTypeInfo(use = Id.MINIMAL_CLASS, include = As.PROPERTY, property = MCioConfig.MCIO_TYPE)
record StatsUpdateOption(ArrayList<StatCategory> categories) implements Option {}

@JsonTypeInfo(use = Id.MINIMAL_CLASS, include = As.PROPERTY, property = MCioConfig.MCIO_TYPE)
record StatsFullOption(ArrayList<StatCategory> categories) implements Option {}
...
record ActionPacket(
    ...
    int sequence,
    String[]
        commands, // Server commands to execute (teleport, time set, etc.). Do not include the /
...
class ActionPacketUnpacker {
  private static final Logger LOGGER = LogUtils.getLogger();
  private static final ObjectMapper CBOR_MAPPER = new ObjectMapper(new CBORFactory());
  private static final ObjectMapper DEBUG_MAPPER =
  ...
  public static Optional<ActionPacket> unpack(byte[] data) {
    try {
      ...
      if (actionPacket.version() != MCioConfig.MCIO_PROTOCOL_VERSION) {
        LOGGER.error(
            "MCIO_PROTOCOL_VERSION mismatch: got {}, expected {}",
            actionPacket.version(),
