...
/**
 * MCIO_MODE=sync refers to the agent and Minecraft being synchronized. However, when in sync mode
 * we also need to synchronize the Minecraft client and server threads. That's what this class is
 * for. Until the game reaches the running state, both threads run freely. Once the game has
 * started, we synchronize the threads' ticks such that the client and server handoff from one to
 * the other. The client thread ticks first, then the server thread ticks, and back around. This
 * isn't a perfect solution since the client won't have the server's updates from the current action
 * when the observation is generated. But at least it will be consistent. Will revisit if necessary.
 * I think the only way to be fully up-to-date when the observation is generated is to run two
 * client ticks and one server tick for every step - Action arrives - client tick - server tick
 * (update the client) - client tick again to integrate server updates, and generate the
 * observation. It would be nice to avoid this.
 *
 * <p>Order of events (once the game is running) MinecraftClient.run() -> game loop ->
 * MinecraftClient.render() [Mojang calls this runTick()] -> MinecraftClient.tick() -> 1.
 * START_CLIENT_TICK -> acquire client sem (Wait for previous server to finish tick) -> wait for
 * action -> process action -> client tick -> continue render() -> frame capture callback ->
 * generateObservation() -> 2. END_CLIENT_TICK -> release server sem 3. START_SERVER_TICK -> acquire
 * server sem 4. END_SERVER_TICK -> release client sem ...
 */
public class MCioSyncUtil {
  ...
  private final TickContext CLIENT_TICK =
      new TickContext("Client", clientTickSem, serverTickSem, true);
  private final TickContext SERVER_TICK =
      new TickContext("Server", serverTickSem, clientTickSem, false);
  ...
  /**
   * Sets the server to frozen. This prevents the world from ticking before the agent is ready. Call
   * at SERVER_STARTED.
   */
  ...
  private record TickContext(
      String label, Semaphore acquireSem, Semaphore releaseSem, boolean incrementCycle) {}
}
